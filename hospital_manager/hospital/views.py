...
def update_doctor_view(request,pk):
    ...
    userForm=models.DoctorUserForm(instance=user)
    doctorForm=models.DoctorForm(request.FILES,instance=doctor)
    mydict={'userForm':userForm,'doctorForm':doctorForm}
    if request.method=='POST':
        userForm=models.DoctorUserForm(request.POST,instance=user)
        doctorForm=models.DoctorForm(request.POST,request.FILES,instance=doctor)
        if userForm.is_valid() and doctorForm.is_valid():
            user=userForm.save()
            user.set_password(user.password)
            user.save()
...
def admin_add_doctor_view(request):
    userForm=models.DoctorUserForm()
    doctorForm=models.DoctorForm()
    mydict={'userForm':userForm,'doctorForm':doctorForm}
    if request.method=='POST':
        userForm=models.DoctorUserForm(request.POST)
        doctorForm=models.DoctorForm(request.POST, request.FILES)
        if userForm.is_valid() and doctorForm.is_valid():
            user=userForm.save()
            user.set_password(user.password)
            user.save()
